...
class AuthService {
  ...
  Stream<List<ConsultationModel>> getOtherUsersConsultations(String userId, {bool isLawyer = false, String? speciality}) {
    return _firestore
        .collection('consultations')
        .snapshots()
        .map((s) {
      final list = s.docs
          ...
          .where((c) {
            // لا يرى استشاراته الخاصة (تظهر في تبويب History)
            if (c.userId == userId) return false;
            // إذا كانت خاصة، تظهر للمحامين فقط
            if (c.isPrivate && !isLawyer) return false;
            // فلترة بالتخصص إذا توفر
            if (speciality != null && c.type != speciality) return false;
            return true;
          })
          .toList();
      list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
  ...
  final docRef = _firestore.collection('consultations').doc(consultationId);
