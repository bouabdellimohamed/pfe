...
class _AIAssistantScreenState extends State<AIAssistantScreen>
  ...
  Future<void> _send([String? text]) async {
    ...
    try {
      ...
      final safetyResult = await GeminiService.validateContentSafety(message);

      if (safetyResult['isSafe'] == false) {
        setState(() {
          _isLoading = false;
          _messages.add(_ChatMessage(
            text: safetyResult['reason'] ?? "Inappropriate content detected.",
            isUser: false,
            timestamp: DateTime.now(),
            isError: true,
          ));
      ...
      // Build system prompt
      final systemPrompt = '''
أنت مستشار قانوني جزائري متخصص. مهمتك:
1. تحليل مشكلة المستخدم وتقديم نصيحة قانونية مختصرة وعملية (3-5 جمل فقط).
2. تحديد الاختصاص القانوني المناسب من القائمة التالية فقط:
[Droit familial, Droit pénal, Droit commercial, Droit civil, Droit immobilier, Droit administratif, Droit du travail, Droit des sociétés, Droit fiscal, Propriété Intellectuelle, Droit bancaire, Droit des assurances, Droit médical, Droit social, Droit de la consommation]

قواعد مهمة:
- إذا كان المستخدم يكتب بالعربية، رد بالعربية. إذا كتب بالفرنسية، رد بالفرنسية.
- كن مختصراً ومفيداً — لا تطوّل.
- إذا لم يكن الموضوع قانونياً، اكتب HORS_SUJET في حقل domain.
- لا تذكر أنك نموذج ذكاء اصطناعي.

أجب دائماً بصيغة JSON فقط بهذا الشكل:
{"advice": "نصيحتك القانونية المختصرة هنا", "domain": "اسم الاختصاص من القائمة"}
''';
      ...
      final isOffTopic = domain == null ||
          domain == 'HORS_SUJET' ||
          !_domains.any((d) => d.toLowerCase() == domain!.toLowerCase());

      final matchedDomain = isOffTopic
          ? null
          : _domains.firstWhere(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) => GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            (_suggestions[i].split('  ').first +
                                '  ' +
                                (_suggestions[i].split('  ').last).tr()),
                            style: GoogleFonts.poppins(
                                fontSize: 12,
