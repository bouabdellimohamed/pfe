      ...
      appBar: AppBar(
        ...
        flexibleSpace: ClipRect(
          child: BackdropFilter(
            ...
            child: Container(color: _navyDark.withOpacity(0.8)),
          ),
        ),
        ...
        child: SafeArea(
          child: TabBarView(
            ...
            children: [
              ...
              _ConsultationsTab(uid: _uid, auth: _auth, specialities: _mySpecialities),
            ],
          ),
        ),
      ),
            ...
            Row(children: [
              ...
              GestureDetector(
                onTap: () => Navigator.push(context, MaterialPageRoute(
                  builder: (_) => ClientProfileScreen(clientId: r.userId),
                )),
                child: Row(
                  children: [
                    const Icon(Icons.person_outline_rounded, size: 14, color: Color(0xFF4FC3F7)),
                    const SizedBox(width: 4),
                    Text(r.userFullName,
                        style: GoogleFonts.poppins(color: const Color(0xFF4FC3F7), fontSize: 12, decoration: TextDecoration.underline, decorationColor: const Color(0xFF4FC3F7))),
                  ],
                ),
              ),
              ...
              GestureDetector(
                onTap: () => SignalementService.showReportDialog(
                  ...
                  reportedUserName: r.userFullName,
                  reportedUserRole: 'user',
                  relatedContentId: r.id,
                  relatedContentType: 'request',
                ),
                child: const Icon(Icons.flag_outlined, size: 16, color: Colors.white38),
              ),
            ]),
...
class _ConsultationsTab extends StatelessWidget {
  ...
  final AuthService auth;
  final List<String> specialities;
  const _ConsultationsTab({required this.uid, required this.auth, required this.specialities});
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<ConsultationModel>>(
      ...
      builder: (ctx, snap) {
        ...
        // ✅ فلترة الاستشارات حسب تخصص المحامي
        if (specialities.isNotEmpty) {
          list = list.where((c) {
            final consType = c.type.toLowerCase().trim();
            return specialities.any((spec) => 
              spec.toLowerCase().trim() == consType || 
              consType.contains(spec.toLowerCase().trim()));
          }).toList();
        }
...
class _ConsultCardState extends State<_ConsultCard> {
  ...
  Future<void> _send() async {
    ...
    try {
      // ✅ AI Content Moderation — مراقبة ردود المحامين
  ...
  Widget build(BuildContext context) {
    ...
    final hasAnsweredByMe = c.answers.any((ans) => ans.lawyerId == widget.lawyerId);
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            GestureDetector(
              onTap: () => Navigator.push(context, MaterialPageRoute(
                builder: (_) => ClientProfileScreen(clientId: c.userId),
              )),
              child: Text(name, style: GoogleFonts.outfit(
                  fontWeight: FontWeight.w600, fontSize: 15, color: const Color(0xFF4FC3F7),
                  decoration: TextDecoration.underline, decorationColor: const Color(0xFF4FC3F7))),
            ),
            Text(c.type.tr(), style: GoogleFonts.poppins(color: _goldLight, fontSize: 12)),
          ])),
          // Report button for consultation
          Container(
        ...
        if (c.answers.isNotEmpty) ...[
          ...
          ...c.answers.map((ans) => Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                  ans.lawyerId == widget.lawyerId 
                      ? 'your_answer'.tr() 
                      : 'lawyer_answer'.tr(namedArgs: {'name': ans.lawyerName}),
                  style: GoogleFonts.poppins(color: const Color(0xFF4CAF50), fontSize: 12, fontWeight: FontWeight.w600)),
              const SizedBox(height: 6),
              Text(ans.answer, style: GoogleFonts.poppins(color: Colors.white, fontSize: 13, height: 1.5)),
            ]),
          )).toList(),
        ],
        if (!hasAnsweredByMe) ...[
