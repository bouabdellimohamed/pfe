...
class _NewConsultationFormState extends State<_NewConsultationForm> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final safetyResult = await GeminiService.validateContentSafety(_questionCtrl.text);
      ...
      if (safetyResult['isSafe'] == false) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(safetyResult['reason'] ?? "Inappropriate content detected."),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: _type,
          isExpanded: true,
          icon: const Icon(Icons.unfold_more_rounded, color: greyText),
          hint: Row(
            children: [
              Icon(Icons.category_rounded, color: primaryColor.withOpacity(0.5), size: 20),
              const SizedBox(width: 12),
              Text('legal_specialty'.tr(), style: const TextStyle(color: greyText, fontSize: 14)),
            ],
          ),
          items: widget.allSpecialities.map((t) {
            IconData icon;
            Color color;
            switch (t) {
              ...
              case 'Droit des sociétés': icon = Icons.business_rounded; color = const Color(0xFF0052D4); break;
              case 'Droit fiscal': icon = Icons.request_quote_rounded; color = const Color(0xFF7C3AED); break;
              case 'Propriété Intellectuelle': icon = Icons.lightbulb_rounded; color = const Color(0xFF7C3AED); break;
              case 'Droit bancaire': icon = Icons.account_balance_rounded; color = const Color(0xFF1D4ED8); break;
              case 'Droit des assurances': icon = Icons.shield_rounded; color = const Color(0xFF0891B2); break;
              case 'Droit médical': icon = Icons.local_hospital_rounded; color = const Color(0xFFDC2626); break;
              case 'Droit social': icon = Icons.people_rounded; color = const Color(0xFF7E22CE); break;
              case 'Droit de la consommation': icon = Icons.shopping_cart_rounded; color = const Color(0xFF15803D); break;
            ...
            return DropdownMenuItem<String>(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  Text(t.tr(), style: const TextStyle(color: darkText, fontSize: 14, fontWeight: FontWeight.w600)),
                ],
              ),
            );
          }).toList(),
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: _isPrivate ? const Color(0xFF0052D4).withOpacity(0.05) : Colors.white,
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                color: _isPrivate ? const Color(0xFF0052D4).withOpacity(0.3) : Colors.grey.shade200,
                width: 1.5,
              ),
            ),
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: _isPrivate ? const Color(0xFF0052D4).withOpacity(0.1) : Colors.grey.shade100,
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Icon(
                    _isPrivate ? Icons.lock_rounded : Icons.lock_open_rounded,
                    color: _isPrivate ? const Color(0xFF0052D4) : Colors.grey.shade500,
                    size: 20,
                  ),
                ),
                const SizedBox(width: 16),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        _isPrivate ? 'only_lawyers_see_msg'.tr() : 'all_can_see_msg'.tr(),
                ...
                children: [
                  ...
                  if (c.isPrivate) ...[
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.lock_rounded, size: 10, color: Color(0xFF0052D4)),
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.gavel_rounded, size: 14, color: Color(0xFF0052D4)),
                const SizedBox(width: 8),
                Text(
                  ans.lawyerName,
                  style: const TextStyle(
...
class _SharedConsultCardState extends State<_SharedConsultCard> {
  ...
  Future<void> _submitAnswer() async {
    ...
    try {
      // ✅ AI Content Moderation — مراقبة الردود من المحامين
      final safetyResult = await GeminiService.validateContentSafety(_answerCtrl.text);
      if (safetyResult['isSafe'] == false) {
        setState(() => _isAnswering = false);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(safetyResult['reason'] ?? 'content_moderation_blocked'.tr()),
              backgroundColor: Colors.red,
            ),
          );
        }
  ...
  Widget build(BuildContext context) {
    ...
    final hasAnsweredByMe = widget.consultation.answers.any((ans) => ans.lawyerId == widget.currentUserId);
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => ConsultationDetailScreen(consultation: widget.consultation),
        ),
      ),
      child: Container(
      decoration: BoxDecoration(
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  if (isLawyer)
                    GestureDetector(
                      onTap: () => SignalementService.showReportDialog(
                        context: context,
                        reportedUserId: widget.consultation.userId,
                        reportedUserName: widget.consultation.userFullName,
                        reportedUserRole: 'user',
                        relatedContentId: widget.consultation.id,
                        relatedContentType: 'consultation',
                      ),
                      child: Container(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (_) => ClientProfileScreen(
                            clientId: widget.consultation.userId,
                          ),
                        ),
                      );
                    } : null,
                    child: Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.all(6),
                          decoration: BoxDecoration(
                            color: isLawyer ? const Color(0xFF0052D4).withOpacity(0.1) : Colors.grey.shade100,
                            shape: BoxShape.circle,
                          ),
                          child: Icon(Icons.person_rounded, size: 14, color: isLawyer ? const Color(0xFF0052D4) : const Color(0xFF64748B)),
                        ),
                        const SizedBox(width: 10),
                        Text(
                          widget.consultation.userFullName,
                          style: TextStyle(
                            color: isLawyer ? const Color(0xFF0052D4) : const Color(0xFF64748B),
                            fontSize: 13,
                            fontWeight: FontWeight.w700,
                            decoration: isLawyer ? TextDecoration.underline : null,
                            decorationColor: const Color(0xFF0052D4),
                          ),
                        ),
                        if (isLawyer) ...[
                          const SizedBox(width: 6),
                          const Icon(Icons.open_in_new_rounded, size: 12, color: Color(0xFF0052D4)),
                        ],
                      ],
                    ),
                  ),
                  ...
                  if (widget.consultation.answers.isNotEmpty) ...[
  const SizedBox(height: 20),
  Column(
    children: widget.consultation.answers.map((ans) {
      return Container(
        margin: const EdgeInsets.only(bottom: 12),
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
