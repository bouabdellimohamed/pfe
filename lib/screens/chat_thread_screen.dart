...
class _ChatThreadScreenState extends State<ChatThreadScreen> {
  ...
  Future<void> _loadOtherPersonName() async {
    ...
    if (mounted) {
      ...
      if (conversation.lastMessageSenderId != uid && !conversation.isRead) {
        await _chat.markAsRead(widget.conversationId);
      }
    }
  }
  ...
  Future<void> _navigateToProfile() async {
    ...
    // Determine the role inside the conversation
    try {
      ...
      final isLawyer = uid == data['lawyerId'];
      
      if (isLawyer) {
        // I am a lawyer, navigate to Client Profile
        Navigator.push(
          context,
          MaterialPageRoute(
        ...
        // I am a user, navigate to Lawyer Profile
        final lawyerDoc = await FirebaseFirestore.instance.collection('lawyers').doc(data['lawyerId']).get();
        if (lawyerDoc.exists && mounted) {
          final lawyer = LawyerModel.fromMap(lawyerDoc.data()!);
          Navigator.push(
            context,
            MaterialPageRoute(
  ...
  Future<void> _send() async {
    ...
    try {
      // ✅ AI Content Moderation — مراقبة رسائل المحادثة
      if (sentText.isNotEmpty) {
        final safetyResult = await GeminiService.validateContentSafety(sentText);
        if (safetyResult['isSafe'] == false) {
          if (mounted) {
            _ctrl.text = sentText; // Restore text
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
