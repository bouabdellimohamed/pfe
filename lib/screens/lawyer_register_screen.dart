...
class _LawyerRegisterScreenState extends State<LawyerRegisterScreen> {
  ...
  void dispose() {
    ...
    _confirmCtrl.dispose(); _phoneCtrl.dispose(); _expCtrl.dispose();
    super.dispose();
  }

  void _onWilayaChanged(String? w) {
    setState(() {
      _wilaya = w; _daira = null; _commune = null;
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    _card(children: [
                      _field(label: 'Nom complet *', ctrl: _nameCtrl,
                          hint: 'Maître Jean Dupont', icon: Icons.person_outline_rounded,
                          validator: (v) => v!.isEmpty ? 'Champ requis' : null),
                      const SizedBox(height: 14),
                      _field(label: 'Email professionnel *', ctrl: _emailCtrl,
                          hint: 'avocat@cabinet.dz', icon: Icons.mail_outline_rounded,
                    ...
                    _sectionTitle('Localisation du cabinet', Icons.location_on_outlined),
                    const SizedBox(height: 14),
                    _card(children: [
                      Text('Aidez les clients à vous trouver en indiquant votre wilaya.',
                          style: TextStyle(color: _textSecondary.withOpacity(0.8),
                              fontSize: 12, fontStyle: FontStyle.italic)),
                      const SizedBox(height: 14),
                      _dropdown(label: 'Wilaya *', value: _wilaya,
                          items: AlgeriaData.wilayaDairas.keys.toList(),
                          hint: 'Sélectionnez votre wilaya',
                          onChanged: _onWilayaChanged,
                          validator: (v) => v == null ? 'Champ requis' : null),
                      const SizedBox(height: 14),
                      _dropdown(label: 'Daïra', value: _daira, items: _dairas,
                          hint: _wilaya == null ? 'Choisissez une wilaya d\'abord' : 'Optionnel',
                          onChanged: _onDairaChanged),
                      const SizedBox(height: 14),
                      _dropdown(label: 'Commune', value: _commune, items: _communes,
                          hint: _daira == null ? 'Choisissez une daïra d\'abord' : 'Optionnel',
                    ...
                    _sectionTitle('Profil professionnel', Icons.gavel_rounded),
                    const SizedBox(height: 14),
                    _card(children: [
                      _field(label: "Années d'expérience", ctrl: _expCtrl,
                          hint: 'ex: 10', icon: Icons.work_outline_rounded,
                      ...
                      Container(
                        ...
                        child: Row(children: [
                          Checkbox(
                            value: _isGeneralist,
                            onChanged: (v) => setState(() => _isGeneralist = v ?? false),
                            activeColor: _gold, checkColor: _navy,
                            side: BorderSide(color: _textSecondary.withOpacity(0.4)),
                          ),
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            Text('Cochez si vous gérez tous types de dossiers',
                      ...
                      Text('Choisissez jusqu\'à 3 domaines principaux.',
                          style: TextStyle(color: _textSecondary.withOpacity(0.7), fontSize: 12, fontStyle: FontStyle.italic)),
                    ...
                    const SizedBox(height: 14),
                    _card(children: [
                      _field(label: 'Mot de passe *', ctrl: _passCtrl,
                      ...
                      _field(label: 'Confirmer le mot de passe *', ctrl: _confirmCtrl,
                          ...
                          suffix: _toggleIcon(_obscureConfirm, () => setState(() => _obscureConfirm = !_obscureConfirm)),
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      const Text('Déjà un compte ?', style: TextStyle(color: _textSecondary, fontSize: 14)),
                      TextButton(
                        onPressed: () => Navigator.pushReplacementNamed(context, '/lawyer-login'),
                        style: TextButton.styleFrom(foregroundColor: _gold),
                        child: const Text('Se connecter', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                      ),
                    ]),
                  ]),
                ),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      GestureDetector(
        ...
        child: Container(
          width: 40, height: 40,
          decoration: BoxDecoration(
            color: _navyLight, borderRadius: BorderRadius.circular(10),
      ...
      Container(
        ...
        child: const Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(width: 6),
          Text('ESPACE AVOCAT', style: TextStyle(color: _gold, fontSize: 11,
              fontWeight: FontWeight.w700, letterSpacing: 2)),
        ]),
      ),
      const SizedBox(height: 12),
      const Text('Créez votre\nprofil professionnel',
          style: TextStyle(color: _textPrimary, fontSize: 26,
              fontWeight: FontWeight.w700, height: 1.25, letterSpacing: -0.5)),
  ...
  Widget _sectionTitle(String title, IconData icon) => Row(children: [
    ...
    Text(title.toUpperCase(), style: const TextStyle(
        color: _gold, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 2)),
  ]);

  Widget _card({required List<Widget> children}) => Container(
    padding: const EdgeInsets.all(20),
    decoration: BoxDecoration(
      color: _navyLight, borderRadius: BorderRadius.circular(16),
      border: Border.all(color: const Color(0x1AC9A84C)),
      boxShadow: [const BoxShadow(color: Color(0x30000000), blurRadius: 18, offset: Offset(0, 6))],
    ),
  ...
  Widget _field({
    ...
    TextFormField(
      ...
      decoration: InputDecoration(
        hintText: hint, hintStyle: const TextStyle(color: Color(0x808A9BB0), fontSize: 13),
        prefixIcon: Icon(icon, color: _textSecondary, size: 20),
        suffixIcon: suffix, filled: true, fillColor: _navy,
  ...
  Widget _dropdown({
    ...
    required String hint, required Function(String?) onChanged,
    ...
    Container(
      ...
      child: DropdownButtonFormField<String>(
        value: value,
        hint: Text(hint, style: const TextStyle(color: Color(0x808A9BB0), fontSize: 13)),
  ...
  Widget _errorBanner(String msg) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(msg, style: const TextStyle(color: Color(0xFFEF9A9A), fontSize: 13))),
    ]),
  );
}
